 ...
 * A {@code BotCommand} wraps a {@link CommandDefinition}{@code <CommandSource>} that declaratively
 * describes the command's name, aliases, description, structured usage tree, visibility, and
 * execution logic.
 *
 * <p>Commands are registered into the {@code CommandRegistry}. The {@code CommandProcessor} strips
 * the global prefix (e.g.
 ...
 * {@link #aliases()}, {@link #description()}, {@link #usage()}, {@link #isVisible()}) derive from
 ...
 * Implementations are wired as Spring beans in application composition configuration and can be
 * toggled via {@code @ConditionalOnProperty} with the prefix {@code lukos.commands.control}.
 *
 * <h2>Visibility & help output</h2>
 * Commands whose {@link #isVisible()} returns {@code false} remain executable by name but are
 * excluded from generic listings such as {@code /help}. This is useful for internal or deprecated
 * commands that should not clutter the help output.
 *
 * <h2>Usage rendering</h2>
 * The {@link #usage()} method constructs a {@link UsageNode} tree from the command's
 * {@code CommandDefinition} via {@link CommandUsageMapper}. The help system ({@link UsageOutput})
 * decides whether to render as plain text or as an image based on output length and the user's
 * requested mode.
 *
 * <h2>Policy & permissions</h2>
 * Permission checks are handled externally by {@code CommandProcessor} via {@code PolicyService}.
 * Command implementations should not perform their own permission checks; they can assume the
...
public interface IBotCommand {
     ...
     * or empty input. This method is used by the command registry for command lookup and by the
     * runtime for verifying the root token.</p>
     *
     ...
     * as the root literal in the usage tree. It should be unique across all registered bot
     * commands.</p>
     *
     ...
     * command registry during command lookup. Using aliases can greatly improve user experience for
     * frequently-used or long command names.</p>
     *
     ...
     * {@link #aliases()}, {@link #description()}, {@link #usage()}, {@link #isVisible()}) derive
     * from the returned definition by default.</p>
     *
    ...
    /**
     * Sends this command's usage/help output to the given source using the specified command prefix
     * and output mode.
     *
     * <p>Delegates to {@link UsageOutput#sendUsage} which automatically decides
     * between text and image rendering based on output size constraints. The mode parameter allows
     * the caller to override this default:</p>
     * <ul>
     ...
     *
     * @param src     message sink for replies.
     * @param prefix  command prefix, e.g. {@code "/"}; {@code null} or blank defaults to
     *                {@code "/"}.
    ...
    default void sendUsage(
        ...
        var mode = UsageOutput.parseMode(modeRaw);
        var opt = UsageTextRenderer.Options.forHelp(
     ...
     * {@code CommandDefinition} by {@link CommandUsageMapper}. It contains syntax lines,
     * parameter/option documentation, examples, notes, and recursive subcommand nodes.</p>
     *
     * <p>The help system ({@link UsageOutput}) renders this tree as text or as an
     * image depending on output length and the user's requested mode. Commands should not implement
     * or call this method directly; use {@link #sendUsage(CommandSource)} to display help to a
     * user.</p>
     *
     * <p>Derived from {@link #definition()} by default.</p>
     *
    ...
    /**
     * Convenience overload that sends usage with the default prefix {@code "/"} and auto mode
     * detection.
     *
     * <p>Equivalent to calling {@link #sendUsage(CommandSource, String, String)
    ...
    /**
     * Returns whether this command should appear in generic help output such as the {@code /help}
     * command listing.
     *
     * <p>A hidden command ({@code isVisible() == false}) remains executable
     * by name and via aliases, but is omitted from {@code /help} output and command list overviews.
     * This is useful for internal or transitional commands.</p>
     *
     * <p>Derived from {@link #definition()} by default.</p>
     *
