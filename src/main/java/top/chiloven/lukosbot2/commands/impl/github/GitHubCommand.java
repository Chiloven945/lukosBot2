...
import static top.chiloven.lukosbot2.commands.UsageNode.*;
...
public class GitHubCommand implements IBotCommand {
    ...
    public UsageNode usage() {
        return UsageNode.root(name())
                ...
                .subcommand("user", "查询用户信息", b -> b
                        .syntax("查询用户信息", arg("username"))
                        .param("username", "GitHub 用户名")
                        .example("github user GitHub")
                )
                .subcommand("repo", "查询仓库信息", b -> b
                        .syntax("查询仓库信息", arg("owner/repo"))
                        .param("owner/repo", "仓库名，格式 owner/repo")
                        .example("github repo Chiloven945/lukosbot2")
                )
                .subcommand("search", "搜索仓库", b -> {
                    Item top = UsageNode.concat(lit("--top="), arg("num"));
                    Item lang = UsageNode.concat(lit("--lang="), arg("lang"));
                    Item sort = UsageNode.concat(lit("--sort="),
                            UsageNode.oneOf(lit("stars"), lit("updated")));
                    Item order = UsageNode.concat(lit("--order="),
                            UsageNode.oneOf(lit("desc"), lit("asc")));
