...
public record Creator(
    ...
    public @NonNull String getString() {
        ...
        sb.append("收录于：").append(indexed.format(TimeUtils.getDTF())).append("\n");
        sb.append("更新于：").append(updated.format(TimeUtils.getDTF())).append("\n\n");

        sb.append("帖子：").append("\n");
        for (int i = 0; i < Math.min(10, posts.size()); i++) {
            sb.append(posts.get(i).getBrief()).append("\n");
        }
        if (posts.size() > 10) {
