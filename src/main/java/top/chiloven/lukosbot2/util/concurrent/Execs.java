...
/**
 * Utility methods for Java 25 concurrency: provides a virtual-thread-per-task executor for highly concurrent I/O or
 * blocking tasks and a named daemon platform-thread factory for single-threaded lanes; this is a non-instantiable
 * helper class.
 */
public final class Execs {
    ...
    /**
     * Creates a virtual-thread-per-task {@link java.util.concurrent.ExecutorService} with a default naming pattern
     * {@code "v-%02d"}, suitable when explicit naming is desired but a custom pattern is not needed.
     *
    ...
    /**
     * Creates a virtual-thread-per-task {@link java.util.concurrent.ExecutorService} whose threads are explicitly named
     * using the given printf-style pattern (e.g., {@code "proc-%02d"} or {@code "send-%02d"}), enabling clearer logs
     * and diagnostics while preserving the lightweight concurrency of virtual threads.
     *
    ...
    /**
     * Returns a {@link java.util.concurrent.ThreadFactory} that creates daemon platform threads named using the given
     * pattern (optionally containing {@code %d} for a monotonically increasing index), useful for single-threaded
